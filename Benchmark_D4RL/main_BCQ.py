...
parser.add_argument('--vae_lr', type=float, default=3e-04)
...
def main(args, log_dir, replay_buffer, vec_env):
    ...
    policy = BCQ(args, state_dim, act_dim, max_action)
    ...
    for ep in tqdm(range(args.epochs)):
        ...
        vae_loss, q_loss, policy_loss = policy.train(replay_buffer, args.itr)
        ...
        print('----------------------------------------------------------------------------------------')
        print('# epoch: {} # avg.reward: {} # Q_loss: {} # Policy_loss: {} # VAE_loss: {}'.format(ep, eval_reward,
                                                                                q_loss, policy_loss, vae_loss))
        print('----------------------------------------------------------------------------------------')

        summary.add_scalar('critic_loss', q_loss, ep)
        summary.add_scalar('policy_loss', policy_loss, ep)
        summary.add_scalar('vae_loss', vae_loss, ep)
        ...
        for d in dataset_list:
            ...
            for seed in seed_list:
                ...
                if args.env not in FLOW:
                    ...
                    log_dir = args.logdir + date + log_dir_name + '_BCQ'
                    os.mkdir(log_dir)
