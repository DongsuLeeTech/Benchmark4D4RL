...
from Algos.OneStepRL import OnestepRL
...
parser.add_argument('--pretrain-bc', type=int, default=500000)
...
# OneStepRL algorithm parameter
parser.add_argument('--q-lr', type=float, default=3e-04,)
parser.add_argument('--bc-lr', type=float, default=3e-04,)
...
def main(args, log_dir, replay_buffer, vec_env):
    ...
    policy = OnestepRL(args, state_dim, act_dim, max_action)

    done = True
    reward_list = []

    train_loss = policy.pretrain_bc(replay_buffer, args.pretrain_bc)

    for ep in tqdm(range(args.epochs)):

        q_loss = policy.q_train(replay_buffer, args.itr)
        ...
        print('----------------------------------------------------------------------------------------')
        print('# epoch: {} # avg.reward: {} # Q_loss: {}'.format(ep, eval_reward, q_loss))
        print('----------------------------------------------------------------------------------------')
        ...
        for d in dataset_list:
            ...
            for seed in seed_list:
                ...
                if args.env not in FLOW:
                    ...
                    log_dir = args.logdir + date + log_dir_name + '_OnestepRL'
                    os.mkdir(log_dir)
